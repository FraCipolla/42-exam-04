...
void	my_exec(char **argv, int i, char **envp)
{
	...
	if (fork() == 0) {
		...
		my_perror("error: cannot execute ", argv[0]);
		exit(-1);
	}
	waitpid(-1, &status, 0);
	if (status == -1)
...
int main(int argc, char *argv[], char **envp)
{
	...
	while (argv[0] && argv[1])
	{
		...
		if (!(strcmp(*argv, "cd"))) {
			if (i != 2)
				my_perror("error: cd: bad arguments", NULL);
			else if (chdir(argv[1]) != 0)
				my_perror("error: cd: cannot change directory to ", argv[1]);
		}
